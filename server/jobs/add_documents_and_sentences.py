...
def create_para_document(src_document_path, tgt_document_path, src_lang, tgt_lang):
...
def add_para_sentences_from_file(sentence_file, src_lang, tgt_lang):
    try:
        n_success = 0
        ...
        return {
            ...
            "message": f"Đã thêm vào {n_success} cặp câu."\
                f"\n{n_error_duplicated} trùng lặp -> không được thêm vào."\
                f"\n{n_error_missing_pair} không đủ cặp (chỉ có vế trái hoặc chỉ có vế phải) -> không được thêm vào."\
                f"\n{len(error_unknowns)} câu gặp lỗi không lường trước -> không được thêm vào. Các lỗi không lường trước: {error_unknown_message}"
        }
    except Exception as err:
        return {
            "success": False,
...
def add_para_documents_from_local(bitextor_path, bitextor_done_path, bitextor_err_path, bitextor_log_path):
    ...
    for language_pair in tqdm(language_pairs, desc="Adding documents"):
        document_path = f"{bitextor_path}/{language_pair}/documents"
        ...
        for prefix in prefixs:
            src_document_path = f"{bitextor_path}/{language_pair}/documents/{prefix}{src_lang}.txt"
            tgt_document_path = f"{bitextor_path}/{language_pair}/documents/{prefix}{tgt_lang}.txt"

            src_done_path = f"{bitextor_done_path}/{language_pair}/documents/{prefix}{src_lang}.txt"
            tgt_done_path = f"{bitextor_done_path}/{language_pair}/documents/{prefix}{tgt_lang}.txt"

            src_err_path = f"{bitextor_err_path}/{language_pair}/documents/{prefix}{src_lang}.txt"
            tgt_err_path = f"{bitextor_err_path}/{language_pair}/documents/{prefix}{tgt_lang}.txt"
            ...
            if not check_dict['success']:
                ...
                success_dict = create_para_document(src_document_path, tgt_document_path, src_lang, tgt_lang)
...
def add_para_sentences_from_local(bitextor_path, bitextor_done_path, bitextor_err_path, bitextor_log_path):
    ...
    for language_pair in tqdm(language_pairs, desc="Adding sentences"):
        sentence_path = f"{bitextor_path}/{language_pair}/sentences"
        ...
        for sentence_filename in all_sentence_files:
            sentence_filepath = f"{bitextor_path}/{language_pair}/sentences/{sentence_filename}"
            sentence_donepath = f"{bitextor_done_path}/{language_pair}/sentences/{sentence_filename}"
            sentence_errpath = f"{bitextor_err_path}/{language_pair}/sentences/{sentence_filename}"
            sentence_logpath = f"{bitextor_log_path}/{language_pair}/sentences/{sentence_filename}"
            ...
            if is_sentence_filename_valid(sentence_filename, langs):
                ...
                src_lang, tgt_lang = get_src_tgt_languages_in_filename(sentence_filename)
                success_dict = add_para_sentences_from_file(sentence_filepath, src_lang, tgt_lang)
